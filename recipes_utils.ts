...
import { ingredient, OneRecipe, ingredientData } from './types'
...
export const renderRecipesMenu = async () => {
    const recipes = $('<div>')
        ...
        .appendTo($('.action'))
    const header = $('<div>')
        .addClass('recipes__header')
        .appendTo(recipes).toggle()

    $('<input>')
        .addClass('recipes__header--text')
        .appendTo(header)
        .attr('placeholder', 'wpisz nazwe przepisu')
        .css('margin-left', '20px')
    $('<div>')
        .addClass('recipes__products')
        .appendTo(recipes)

    const button = $('<button>')
        .addClass('recipes__header--button')
        .appendTo(header)
        .text('add')
    const recipeBox = $('<div>')
        .addClass('recipe-Box')
    ...
    ingredients.forEach(object => {
        const ingredientDiv = $('<div>')
            .appendTo($(`.recipes__products`))
            .addClass('recipes__products--div')
            .attr('ingredientsId:', object.ingredientsId)
            .text(object.name)
...
export const renderRecipes = (name: string, id: number, array: Array<ingredient>) => {
    ...
    const readyRecipe = $('<div>')
        .addClass('ready-recipe')
        .appendTo(recipesBox)
        .attr('recipesId:', id)
    const trashIcon = $('<button>')
        .appendTo(readyRecipe)
        .addClass('ready-recipe__trash')

    $('<div>')
        .addClass('ready-recipe__title')
        .appendTo(readyRecipe).text(`Name: `)
        .css('font-weight', 'bold')
        .css('justify-content', 'center')
    $('<div>')
        .addClass('ready-recipe__name')
        .appendTo(readyRecipe)
        .text(name)
    $('<div>')
        .addClass('ready-recipe__product')
        .appendTo(readyRecipe)
        .text('Products: ')
        .css('font-weight', 'bold')
        .css('justify-content', 'center')
    ...
    array.forEach(product => {
        $('<div>')
            ...
            .attr('ingredientId:', product.ingredientId)
    })
